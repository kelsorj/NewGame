...
export const MapEditor3DCanvas = ({ onBackToGame }) => {
    ...
    const updateRoomData = async (roomId, name, description, items) => {
        try {
            const response = await fetch(`${API_BASE}/room-data`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ roomId, name, description, items })
            });
                            ...
                            <div style={{ marginBottom: '5px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <input
                                        ...
                                        onBlur={(e) => updateRoomData(selectedRoom.id, e.target.value, selectedRoom.description, selectedRoom.items)}
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                                e.target.blur();
                                            }
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                                ...
                                {!editingDescription ? (
                                    ...
                                    <div>
                                        <button
                                            onClick={async () => {
                                                const success = await updateRoomData(selectedRoom.id, undefined, editDescription, undefined);
                                                if (success) {
                                    ...
                                    <div>
                                        <button
                                            onClick={async () => {
                                                ...
                                                const success = await updateRoomData(selectedRoom.id, undefined, undefined, itemsArray);
                                                if (success) {
